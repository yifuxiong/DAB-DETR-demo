...
class MultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None,
        ...
        # if kdim and vdim are None, they will be set to embed_dim such that
        # query, key, and value have the same number of features.
...
def multi_head_attention_forward(query: Tensor,
    ...
    # q.size = query.size = [tgt_len, bsz, embed_dim]
    ...
    # torch.bmm() -> 固定3维矩阵相乘
    # a = torch.tensor(bs, h, w)
    # b = torch.tensor(bs, w, m)
    # size(torch.bmm(a, b)) = [bs, h, m]
    attn_output_weights = torch.bmm(
        q, k.transpose(1, 2)
    )  # q * k^T, size = [bs * num_heads, target_sequence_len, source_sequence_len]
