...
def gen_sineembed_for_position(pos_tensor):
    '''
    generate sine embed for position
    '''
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.return_intermediate:
            if self.bbox_embed is not None:
                return [
                    torch.stack(intermediate).transpose(1, 2),  # (6,100,2,256)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        # q(768,2,256), k(768,2,256), v(768,2,256)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        '''
        :param d_model: 默认是512，下面用256代入计算
        :param nhead: 默认是8
        :param dim_feedforward: 2048
        :param dropout:
        :param activation:
        :param normalize_before:
        :param keep_query_pos:
        :param rm_self_attn_decoder:
        '''
        super().__init__()
    ...
    def forward(self, tgt, memory,
        ...
        # (100, 2, 16, 32) -> (100, 2, 256 * 2 = 512)
        q = torch.cat([q, query_sine_embed], dim=3).view(num_queries, bs, n_model * 2)
        # (768, 2, 8, 256 // 8 = 32)
        k = k.view(hw, bs, self.nhead, n_model // self.nhead)
        # (768, 2, 8, 256 // 8 = 32)
        k_pos = k_pos.view(hw, bs, self.nhead, n_model // self.nhead)
        # (768, 2, 16, 32) -> (768, 2, 256 * 2 = 512)
        k = torch.cat([k, k_pos], dim=3).view(hw, bs, n_model * 2)

        # q和上面一样，来自self.attn的输出，k,v来自encoder的输出memory
        # q(100,2,512), k(768,2,512), v(768,2,512)
